...
class MTLSClientHandler:
    """
    Questa classe gestisce l'handshake mTLS (mutual TLS) dal lato del client.
    """
    def __init__(self, user, myfolder, server):
        """
        Inizializza il gestore mTLS con l'utente, il nome della cartella e il server forniti.
    ...
    def handshakeStep(self, step, params=None):
        """
        Esegue uno specifico passo dell'handshake mTLS.

        Parameters:
            step (int): Il numero del passo dell'handshake da eseguire (1, 2 3 o 4).
        ...
        if step == 1:
            ...
            self._step4SendClientCertificate()

    def _step1Handshake(self):
        """
        Esegue il primo passo dell'handshake di mano mTLS.
        """
    ...
    def _step2KeyDerivation(self, params):
        """
        Esegue il secondo passo dell'handshake mTLS.
    ...
    def _step3MessageDecryptionAndVerification(self, params):
        """
        Esegue il terzo passo dell'handshake mTLS.
        ...
        if CertificateUtils.verifyCertificate(serverCertificate, Constants.CA_FILE_PATH):
            ...
            if self._keyUtils.verifySign(decryptedMessage, decryptedSignature, self._pks):
                ...
                print('[User {}]: '.format(self._user), 'Certificato del server verificato!')
                time.sleep(1)
                self.handshakeStep(4)

            else:
                print('[User {}]: '.format(self._user), 'certificato del server non verificato!')
                self._user.closeConnection()
                return

    def _step4SendClientCertificate(self):

        print('[User {}]: '.format(self._user), 'Invio del certificato del client...')
        clientCertFile = self._myfolder + Constants.CERTIFICATE_FILENAME
        ...
        if not self._user.sign_CIE(message, self._myfolder + Constants.ECDSA_KEY_FILENAME, signatureFile):
            self._user.closeConnection()
            return
        ...
        com = [Constants.OPENSSL, 'enc', '-e', '-aes-256-ctr', '-K', self.keys[0].hex(), '-iv',
               self.iv.hex()]
        ...
        self._server.tlsHandler.step3VerifyClientCertificate(self._user, {
            'user': self._user,
...
class MTLSServerHandler:
    """
    Questa classe gestisce l'handshake mTLS dal lato del server.
    """
    ...
    def _step1GenerateBAndK(self, params, callerSelf):
        """
        Esegue il primo passo dell'handshake mTLS.
    ...
    def step3VerifyClientCertificate(self, user, params):
        """
        Verifica il certificato del client.

        Args:
            user (str): L'utente che sta eseguendo l'handshake.
            params (dict): I parametri ricevuti dal client, inclusi il certificato cifrato del client.
        """
        print('[Server {}]: '.format(self._server), 'Verifica del certificato del client...')
        time.sleep(1)
        com = [Constants.OPENSSL, 'enc', '-d', '-aes-256-ctr', '-K', self._connections[user][0].hex(), '-iv',
               params['iv']]
        decryptedMessage = subprocess.check_output(com, input=params['encryptedMessage']).decode().strip()

        pubkey_client = decryptedMessage.split('\n')[0].strip()
        clientCertificate = decryptedMessage.split('\n')[1].strip()

        if CertificateUtils.verifyCertificate(clientCertificate, Constants.CA_FILE_PATH):
            com = [Constants.OPENSSL, 'enc', '-d', '-aes-256-ctr', '-K', self._connections[user][0].hex(), '-iv', params['iv']]
            decryptedSignature = subprocess.check_output(com, input=params['encryptedSignature']).decode().strip()

            # L'utente firma con la CIE l'hash del msg
            com = [Constants.OPENSSL, 'dgst', '-sha256']
            decryptedMessage_hash = subprocess.check_output(com, input=decryptedMessage.encode('utf-8')).decode('utf-8').strip()

            if self._keyUtils.verifySign(decryptedMessage_hash, decryptedSignature, pubkey_client):
                os.remove(decryptedSignature)
                print('[Server {}]: '.format(self._server), 'Certificato del client verificato!')
                time.sleep(1)
                print('[Server {}]: '.format(self._server), 'Handshake mTLS completato')
                time.sleep(1)
            else:
                print('[Server {}]: '.format(self._server), 'Certificato del client non verificato!')
                self._server.closeConnection(params['user'])
                return
